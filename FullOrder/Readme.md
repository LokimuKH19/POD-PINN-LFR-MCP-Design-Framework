...
- I found that the POD-PINN applied 4 different networks and co-training them to regress the P, Ut, Ur, Uz respectively. However, the `TRAIN_FullOrder.py` used only one model to regress those physical quantities in one go, and only accurately predict the Ut successfully. Is this difference could be the possible reason for the failure of the Full-Order surrogate models? To verify this, a new hyperparameter `outputmode` is added to the script and if it is set as `"seperate"`, the four flow field components would be fitted separately by 4 independent neural networks. The new results is added to the `./FullOrderReconstruct` filefolder, and no significant improvements of predicting P and Uz is observed.
