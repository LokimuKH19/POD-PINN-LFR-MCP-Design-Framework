...
> However, when addressing the specific problem, DeepONet isn't the best choice for the positions is sampled via unstructural meshes from CFD data. When the branch net try to perform transformation on the field detected by trunk net, the randomly distributed spatial positions would made this process less effective. Additionally, DeepONet aims to encode the field $y$ by the trunk net directly, but due to this irregularity we have to use another input layer to encode the positions and generate the field in advance, which also appears to limit its performance.
