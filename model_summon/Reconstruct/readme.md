...
Or you might train all $k$ modes simultaneously by letting the neural network output multiple functions: $N(x;\theta)=[\phi^{(1)}_\theta_1(x), \phi^{(2)}_\theta_2(x), \dots, \phi^{(k)}_\theta_k(x)]$.
