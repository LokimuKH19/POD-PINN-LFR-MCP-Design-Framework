...
from Interpolator import ModalNet, Normalizer
...
class PINNSystem:
    def __init__(self, hidden_dim=64, hidden_layers=2, lr=1e-3, use_physics_loss=True, use_physics_batch=False):
        ...
        # ===== 4. Interpolators change to NN =====
        # Load input coords and normalize
        coord_data = pd.read_csv('./coordinate.csv', header=None).values.astype(np.float32)
        self.coord_tensor = torch.tensor(coord_data, dtype=torch.float32).to(self.device)

        self.input_normalizer = Normalizer()
        input_norm_stats = np.load(f'./Interpolator/InputNormalizer_P.npz')
        self.input_normalizer.mean = input_norm_stats['mean']
        self.input_normalizer.std = input_norm_stats['std']
        X_norm = self.input_normalizer.transform(coord_data)
        X_norm_tensor = torch.tensor(X_norm, dtype=torch.float32).to(self.device)

        # Load trained interpolators
        self.interp = {}
        for name in ['P', 'Ut', 'Ur', 'Uz']:
            normalizer = Normalizer()
            stats = np.load(f'./Interpolator/Normalizer_{name}.npz')
            ...
            model_path = f'./Interpolator/Modal_{name}.pth'
            model.load_state_dict(torch.load(model_path, map_location=self.device))
            model.eval()

            # Wrap prediction as callable function
            def make_interp_fn(model):
                return lambda coords: model.predict(coords, denormalize=False, track_grad=True)

            self.interp[name] = make_interp_fn(model)
    ...
    def __call__(self, coords, condition, enable_grad=False):
        ...
        with context():
            for name, net in zip(['P', 'Ut', 'Ur', 'Uz'],
                ...
                X_norm = (coords - torch.tensor(self.input_normalizer.mean, device=coords.device)) / torch.tensor(
                    self.input_normalizer.std, device=coords.device)
                mode_values = self.interp[name](X_norm)  # (M, 4)
    ...
    def compute_physics_residuals(self, coords, condition):
        ...
        return physics_loss/1e17    # to keep balance with data loss
...
# save model
def save_checkpoint(system: PINNSystem, seed: int, path: str = "Reconstruct"):
    checkpoint = {
...
def train_PINN(system: PINNSystem, epochs: int = 9999, coord_batch_size: int = 64, pretrain_epoch: int = 0,
               use_scheduler: bool = True, scheduler_patience: int = 5, scheduler_factor: float = 0.75,
    ...
    for ep in range(1, epochs + 1):
        ...
        if system.use_physics_batch:
            ...
            # This Save
            save_checkpoint(system, seed=SEED, path='./Reconstruct/Step')
