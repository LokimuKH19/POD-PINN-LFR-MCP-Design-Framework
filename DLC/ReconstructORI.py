...
class PINNSystem:
    def __init__(self, hidden_dim=64, hidden_layers=2, lr=1e-3, use_physics_loss=True, use_physics_batch=False, modes=4):
        ...
        self.coords_all += 1e-6    # shift
        # Material constants
        self.rho = 10650   # kg/m3
        self.mu = 0.006  # Pa·s
        self.g = 9.81
    ...
    def step_with_physics_batches(self, coord_batch_size=64):
        ...
        for i in range(num_batches):
            ...
            print(f"    [Batch {i + 1}/{num_batches}] Train Loss: {train_loss:.6f}, "
                  f"Test Loss: {test_loss:.6f}, Physics Loss: {physics_loss.item():.6f}")
...
def load_checkpoint(path: str):
    ...
    pattern = r"SEED(?P<seed>\d+)_LR(?P<lr>[\d\.eE+-]+)_HD(?P<hd>\d+)_HL(?P<hl>\d+)_MO(?P<modes>\d+)_Epoch(?P<epoch>\d+)_WithPhysics(?P<phys>\d+)_WithBatch(?P<batch>\d+)"
    match = re.search(pattern, filename)
    if not match:
        raise ValueError(f"[ERROR] ❌ Cannot parse hyperparameters from filename: {filename}")
    parsed = match.groupdict()
    modes = int(parsed['modes'])
    ...
    system = PINNSystem(
        ...
        modes=len(checkpoint['coeffs_stats']['P'][0]) if not modes else modes
    )
...
def save_loss_curve(train_loss, test_loss, physics_loss, SEED, learning_rate, hidden_dim,
                    hidden_layers, modes, use_physics_loss, use_physics_batch, pretrain,
                    save_dir="ReconstructORI"):
    epochs = len(train_loss)
    os.makedirs(save_dir, exist_ok=True)

    filename_base = (f"Losscurve_SEED{SEED}_LR{learning_rate}_HD{hidden_dim}_"
                     f"HL{hidden_layers}_MO{modes}_Epoch{epochs}_WithPhysics{int(use_physics_loss)}"
                     f"_WithBatch{int(use_physics_batch)}"
                     f"{'' if not pretrain else '_Pretrain'+str(pretrain)}")

    # 保存图像
    plt.figure(figsize=(10, 6))
    # Ensure losses are detached and on CPU
    train_loss = [float(x.detach().cpu().item() if torch.is_tensor(x) else x) for x in train_loss]
    test_loss = [float(x.detach().cpu().item() if torch.is_tensor(x) else x) for x in test_loss]
    ...
    plt.title("Training, Test, and Physics Loss Curve")
    plt.legend()
...
def train_PINN(system: PINNSystem, epochs: int = 9999, coord_batch_size: int = 64, pretrain_epoch: int = 0,
    ...
    Returns:
        ...
        phy_loss_history (list): Physic Loss per epoch
    """
    train_loss_history = []
    test_loss_history = []
    phy_loss_history = []
...
if __name__ == "__main__":
    ...
    # ===== Train =====
    print(f"[INFO] Starting training with {MODES} modes...")
    ...
    # ===== Reload Checkpoint & Test Prediction =====
    try:
        if not pretrain_epochs:
            path = (f"{checkpoint_path}/Checkpoint_SEED{SEED}_LR{learning_rate}_HD{hidden_dim}_"
                    f"HL{hidden_layers}_MO{MODES}_Epoch{epochs}_WithPhysics{int(use_physics_loss)}_"
                    f"WithBatch{int(use_physics_batch)}.pth")
        else:
            path = (f"{checkpoint_path}/Checkpoint_SEED{SEED}_LR{learning_rate}_HD{hidden_dim}_"
                    f"HL{hidden_layers}_MO{MODES}_Epoch{epochs}_WithPhysics{int(use_physics_loss)}_"
                    f"WithBatch{int(use_physics_batch)}_Pretrain{pretrain_epochs}.pth")

        pinn = load_checkpoint(path)
